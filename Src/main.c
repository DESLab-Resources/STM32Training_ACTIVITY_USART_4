...
int main(void)
{
	...
	for(;;)
	{
		uint8_t bt1_state = (GPIOB->IDR & (1 << 1)); // read current state of PB1

		if(!bt1_state) // if button is pushed
		{
			while(!(GPIOB->IDR & (1 << 1)));
			USART1_sendString("Hello");
		}

		uint8_t bt2_state = (GPIOB->IDR & (1 << 2)); // read current state of PB2

		if(!bt2_state) // if button is pushed
		{
			while(!(GPIOB->IDR & (1 << 2)));
			USART1_sendString("Bye");
		}
	}
}
...
void USART1Init()
{
	...
	USART1->BRR = 1250; // 72M/57600 = 1250
...
void GPIO_Config (void)
{
	...
	GPIOB->CRL &= ~((0b11 << 4) | (0b11 << 8)); // clear bit 4 and 5

	// Clear  CNF1[1:0] = 01: Input floating
	GPIOB->CRL |= ((0b11 << 6) | (0b11 << 10)); // Set bit 6
	GPIOB->CRL &= ~((0b11 << 7) | (0b11 << 11)); // Clear bit 7
}
